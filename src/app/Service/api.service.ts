...
export class ApiService {
  ...
  private GET_USERS_API = "http://localhost:8087/admin/users";
  private SRCH_PRD_API = "http://localhost:8087/admin/search";
  ...
  addProduct(auth: string, desc: string,
    quan: string, price: string, prodname: string, image: File, manufacturer: string, category: string): Observable<any> {
  ...
  // delete Product for Logged Admin User
  search(auth: string, searchBy: string, search: string) {
    const myheader = new HttpHeaders().set('AUTH_TOKEN', auth);
    const formData: FormData = new FormData();
    formData.append('searchBy', searchBy);
    formData.append('search', search);
    return this.http.post<any>(this.SRCH_PRD_API, formData,  { headers: myheader });
  }
  ...
  updateProduct(auth: string, desc: string,
    quan: string, price: string, prodname: string, image: File, productid: any, manufacturer: string, category: string): Observable<any> {
  ...
  getUsers(auth: string) {
    const myheader = new HttpHeaders().set('AUTH_TOKEN', auth);
    return this.http.get<any>(this.GET_USERS_API, { headers: myheader });
  }
